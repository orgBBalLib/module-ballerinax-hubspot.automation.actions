...
# Complete definition of an automation action including its configuration and behavior.
public type PublicActionDefinition record {
    ...
    # The unique identifier for the current revision.
    string revisionId;
    ...
    # Array of rules governing action execution and translation.
    PublicExecutionTranslationRule[] executionRules?;
    # The unique identifier for the action definition.
    string id;
    # Array of CRM object types this action supports.
    string[] objectTypes;
...
# Defines a conditional dependency where field visibility depends on a specific field value.
...
# Represents the context for an agent-initiated automation request.
...
# Schema definition for a 64-bit integer field with optional min/max constraints.
...
# Defines display labels and descriptions for an automation action's UI elements.
...
# Identifies an action execution by its enrollment ID and execution order index.
...
# Request context for standalone automation actions with AI context support.
...
# Request context indicating the request originated from the Copilot feature.
...
# Context object for test requests containing the source identifier.
public type TestRequestContext record {
    # Indicates the source of the test request, with the only accepted value being 'TEST'
    "TEST" 'source = "TEST";
};

# Defines the structure and type configuration for a field.
...
# Response indicating if an action definition requires an object.
...
public type ChirpAiContextObject record {
    ...
    string inferenceId?;
    # Array of unstructured data source types used in the AI context.
    ...
    # Indicates whether the AI context data is private.
    boolean isPrivate;
...
# Identifies a contact using portal ID and optional vid or email.
...
public type PublicObjectRequestOptions record {
    # Array of property names to retrieve for the object.
    string[] properties;
};
...
# Defines an input field's requirements, type, and supported value configurations.
...
# Request context containing workflow-specific identifiers and action details.
...
# Batch request for completing multiple callbacks with outputs and context.
...
# Configuration object for creating or updating a public action definition.
public type PublicActionDefinitionEgg record {
    ...
    # The URL endpoint where the action is executed
    string actionUrl;
    # Array of dependencies between input fields that control field behavior.
    PublicActionDefinitionInputFieldDependencies[] inputFieldDependencies?;
