...
public type PublicActionDefinition record {
    ...
    # The revision identifier for version tracking of the action.
    string revisionId;
...
# Schema definition for a 64-bit integer field with optional min/max bounds.
...
# Request payload for completing a callback with output fields and context information.
...
# Paginated collection response containing public action definitions.
...
# Context object for test requests indicating the source of the test invocation.
...
# Represents a selectable option with a label and value for field definitions.
...
# Response indicating whether a custom action definition requires an object context.
...
# Schema definition for a double-precision floating point field with bounds.
...
# Defines the structure and requirements for an input field in an automation action.
...
public type CollectionResponsePublicActionFunctionIdentifierNoPaging record {
    # Array of public action function identifiers in the response.
...
# Paginated collection response containing public action revisions.
...
# Batch request payload for completing multiple callbacks with outputs and context.
...
# Input schema for creating or updating a public action definition.
public type PublicActionDefinitionEgg record {
    ...
    PublicActionDefinitionInputFieldDependencies[] inputFieldDependencies?;
    # Array of rules that define how action execution results are translated.
    PublicExecutionTranslationRule[] executionRules?;
