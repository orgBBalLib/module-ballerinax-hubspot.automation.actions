...
# Complete definition of an automation action including inputs, outputs, and functions.
public type PublicActionDefinition record {
    ...
    # The unique identifier for the current revision of the action.
    string revisionId;
    # Unix timestamp when the action was archived, if applicable.
    int archivedAt?;
    # Array of dependencies between input fields.
    PublicActionDefinitionInputFieldDependencies[] inputFieldDependencies?;
    # Array of rules that govern action execution and translation.
    ...
    string[] objectTypes;
    # Configuration options for object requests specifying desired properties.
    PublicObjectRequestOptions objectRequestOptions?;
};

# Defines a conditional dependency between fields based on a specific field value.
...
# Context object for agent-initiated requests with AI context information.
...
# Schema definition for a 64-bit integer field with optional bounds.
...
# Localized labels and descriptions for an automation action's UI elements.
...
# Identifies a specific action execution within an enrollment by index and enrollment ID.
...
public type FieldTypeDefinition record {
    ...
    string name;
    # Array of available options for selection-based fields.
    Option[] options;
...
# Request context for standalone automation actions not tied to workflows.
...
# Identifies a function associated with an action by type and unique identifier.
...
# Request payload for completing a callback with output fields and typed outputs.
...
# Paginated collection of public action definitions with forward paging support.
...
# Request context indicating the request originates from Copilot.
...
# Request to specify whether an action definition requires an associated object.
...
# Schema definition for a string field with optional format specification.
...
# Container for batch callback completion requests to process multiple callbacks.
...
# Defines an output field returned by an action after execution.
...
# Context object for test requests with a required source identifier.
public type TestRequestContext record {
    # Indicates the source of the test request, with the only accepted value being 'TEST'
    "TEST" 'source = "TEST";
};

# Defines field type properties including data type, options, and display settings.
public type PublicFieldTypeDefinition record {
    ...
    string name;
    # Array of selectable options available for this field.
...
# Defines a function's source code and execution type for automation actions.
...
# Represents a selectable option with a label and value pair.
...
# Union type representing either a single field or conditional field dependency.
...
# Identifies a contact by portal ID and optional vid or email.
...
# Schema definition for a double-precision floating-point field.
...
public type PublicObjectRequestOptions record {
    # Array of property names to include in the request response.
    string[] properties;
};

# Defines a simple dependency where fields depend on a controlling field.
...
# Defines the structure and requirements of an input field for automation actions.
...
public type CollectionResponsePublicActionFunctionIdentifierNoPaging record {
    # Array of public action function identifiers returned in the response.
...
# Request context containing workflow-specific information for action execution.
...
# Represents a specific revision of an action with its definition and metadata.
...
# Paginated collection of action revisions with forward paging support.
public type CollectionResponsePublicActionRevisionForwardPaging record {
    # Contains pagination information for navigating forward through result sets.
    ForwardPaging paging?;
    # Array of action revisions returned in the response.
    PublicActionRevision[] results;
};

# Batch request payload for completing multiple callbacks with outputs.
...
# Template for creating a new public action definition with all required configuration.
public type PublicActionDefinitionEgg record {
    ...
    # The URL endpoint where the action is executed
    string actionUrl;
    # Array of dependencies between input fields that control field visibility or behavior.
    PublicActionDefinitionInputFieldDependencies[] inputFieldDependencies?;
...
# Schema for partially updating an existing action definition with modified properties.
public type PublicActionDefinitionPatch record {
    # Array of input field definitions to update for the action.
    PublicInputFieldDefinition[] inputFields?;
    # Array of output field definitions to update for the action.
    OutputFieldDefinition[] outputFields?;
    # The URL endpoint where the action is executed
    string actionUrl?;
    # Array of input field dependencies to update for the action.
    PublicActionDefinitionInputFieldDependencies[] inputFieldDependencies?;
    # Array of execution translation rules to update for the action.
    PublicExecutionTranslationRule[] executionRules?;
    # Indicates whether the action is published and available for use
    boolean published?;
    # Array of object types to update for the action.
    string[] objectTypes?;
...
# Schema definition for object-type fields containing nested properties.
