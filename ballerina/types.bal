...
# Complete definition of an automation action including its configuration.
public type PublicActionDefinition record {
    ...
    string actionUrl;
    # Indicates whether the action definition is published.
    boolean published;
    ...
    # The revision identifier for this action definition version.
    string revisionId;
    # Unix timestamp when the action was archived.
    int archivedAt?;
    # Array of dependencies between input fields.
    PublicActionDefinitionInputFieldDependencies[] inputFieldDependencies?;
    # Array of rules governing action execution translation.
    PublicExecutionTranslationRule[] executionRules?;
    # The unique identifier for the action definition.
    string id;
    # Array of CRM object types this action applies to.
    string[] objectTypes;
    # Configuration options specifying which properties to include in object requests.
...
# Contains display labels and descriptions for an action and its input/output fields.
...
# Contains pagination information for navigating to subsequent result pages.
...
public type FieldTypeDefinition record {
    ...
    string name;
    # Array of selectable options available for the field.
    Option[] options;
...
# Request context for standalone automation actions not associated with workflows.
...
# Identifies a function associated with an action by its type and unique identifier.
...
public type CollectionResponsePublicActionDefinitionForwardPaging record {
    ...
    # An array of public action definitions returned in the response.
...
# Request context indicating the request originates from the Copilot source.
...
# Schema definition for an array-type field containing typed elements.
...
# Context object for test requests indicating the request source.
public type TestRequestContext record {
    # Indicates the source of the test request, with the only accepted value being 'TEST'
    "TEST" 'source = "TEST";
};

# Defines the structure and type of a field in an automation action.
...
# Defines a function associated with an automation action.
...
# Represents a selectable option with a label and value.
...
# Response indicating whether an action requires an associated object.
...
public type ChirpAiContextObject record {
    ...
    string inferenceId?;
    # Array of source types indicating where unstructured data originates.
...
# Identifies a contact using portal ID, video ID, or email address.
...
# Request context containing workflow-specific information for automation actions.
...
public type ComplianceIds record {
    ...
    # An array of contact identifiers for compliance tracking.
...
public type CollectionResponsePublicActionRevisionForwardPaging record {
    # Contains pagination information for navigating to subsequent result pages.
    ForwardPaging paging?;
    # An array of public action revisions returned in the response.
    PublicActionRevision[] results;
};

# Batch request payload for completing multiple callbacks with their outputs.
...
# Template for creating a new public action definition with all required properties.
public type PublicActionDefinitionEgg record {
    # An array of input field definitions required by the action.
    PublicInputFieldDefinition[] inputFields;
    # An array of output field definitions produced by the action.
    OutputFieldDefinition[] outputFields?;
    # The timestamp indicating when the action was archived
    int archivedAt?;
    # An array of functions associated with the action.
    PublicActionFunction[] functions;
    # The URL endpoint where the action is executed
    string actionUrl;
    # Array of dependencies between input fields for conditional logic.
    PublicActionDefinitionInputFieldDependencies[] inputFieldDependencies?;
...
# Request body for partially updating an existing automation action definition.
...
# Schema definition for a boolean-type field holding true or false values.
...
# Schema definition for an object-type field with nested properties.
